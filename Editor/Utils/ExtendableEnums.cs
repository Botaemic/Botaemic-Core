...
public class ExtendEnumDrawer : PropertyDrawer
{
    ...
    private static List<string> _enumNames;
    private static SerializedProperty _currentProperty = null;
    
    private static bool _showWindow = false;
    private static int _windowWidth = 150;
    private static int _windowHeight = 90;
    ...
    public class NewValuePopup : PopupWindowContent
    {
        public override void OnGUI(Rect position)
        {
            ...
            {
                List<string> names = _enumNames;
                for (int i = 0; i < names.Count; i++)
                {
                    names[i] = names[i].ToLower();
                }
                if (!names.Contains(_newValue.ToLower()))
                {
                    FindClassFile(GetEnumName(_currentProperty), _newValue);
                    this.editorWindow.Close();
                }
                else
                {
                    _newValue = _newValue + "_Copy";
                }
            }
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (!_showWindow)
        {
            if (_enumNames.Count != 0)
            {
                _enumNames.Add("Add New...");
                int newValue = EditorGUI.Popup(position, property.displayName, property.intValue, _enumNames.ToArray());

                if (newValue == _enumNames.Count - 1)
                {
                    NewValuePopup popup = new NewValuePopup();
                    PopupWindow.Show(new Rect(((Screen.width / 2) - (_windowWidth / 2)), position.y - (_windowHeight / 2), 0, 0), popup);
